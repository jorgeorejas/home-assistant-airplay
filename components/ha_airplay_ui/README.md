...
| Long press (≥1.5 s held) | `on_button_long` | Toggles `dac_set_power_mode(STANDBY↔ON)` + `ha_airplay_leds_set_muted()` | None |
...
`ha_airplay_ui_init()` registers a callback via `rtsp_events_register()` (upstream observer pattern). Mapping:

| Event | LED action |
|---|---|
| `RTSP_EVENT_CLIENT_CONNECTED` | `ha_airplay_leds_flash_connection()` (5 s sweep) + state → CONNECTED |
...
|---|---:|---:|:---:|
| `ha_airplay_enc` | 3072 B | 10 | any |
| `ha_airplay_btn` | 3584 B | 8 | any |
| `ha_airplay_sw` (mute slide) | 3072 B | 6 | any |
| `ha_airplay_volpoll` | 3072 B | 4 | any |
