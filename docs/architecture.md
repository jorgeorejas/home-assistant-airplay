...
DenAir is a fork of [rbouteiller/airplay-esp32](https://github.com/rbouteiller/airplay-esp32) (AirPlay 2 receiver for ESP32-S3) with the following DenAir-specific additions:
...
```
                      ...
                      └───────┬──────────────────┬───────────────┘
                        ...
                        iot_board_init         playback events
                              │                   │
  ┌───────────────────────────┼───────────────────┼─────────────────────────┐
  │ components/ (DenAir additions on top of airplay-esp32's DAC abstraction)│
  │                           │                   │                         │
  │      ┌────────────────────▼────────────────┐  │                         │
  │      │ boards/ha_voice_pe                  │  │                         │
  ...
  │              │                                                            │
  │    ┌─────────▼───────────┐    ┌────────────────────┐    ┌──────────────┐ │
  │    │ denair_ui            │◄──┤ rtsp_events        ├───►│ denair_leds  │ │
...
3. `playback_task` resamples if needed, applies digital volume curve, then **before** writing I²S:
   ...
   - calls `denair_audio_tap()` (our bass-band RMS + beat detector)
...
5. `denair_leds` render task runs at 50 Hz on core 1. It reads the tap's atomic energy / beat values, plus the artwork-derived hue, and renders one of six states (mute / volume / connection-in / connection-out / playing / idle).
6. RTSP session events (`CLIENT_CONNECTED`, `PLAYING`, `PAUSED`, `DISCONNECTED`) are subscribed via `rtsp_events_register()` by `denair_ui/ui.c`, which drives `denair_leds_set_playback_state()` and `denair_leds_flash_connection()`.
7. Artwork `SET_PARAMETER` (`image/jpeg`) is forwarded to `denair_artwork_update()`. That copies the JPEG into PSRAM and posts to a decoder task; the task runs tjpgd (ESP32-S3 ROM) at 1/8 scale, sums RGB across the decoded rectangles, converts mean → HSV, and calls `denair_leds_set_base_hue()`.
...
| `denair_leds`           | 1    | 3        | 50 Hz pixel render                    |
| `denair_art`            | 0    | 3        | JPEG decode on track change           |
| `denair_enc`            | any  | 10       | Quadrature state machine              |
| `denair_btn`            | any  | 8        | Multi-click + long-press              |
| `denair_sw` (LED power) | any  | 6        | Mute slide state change               |
| `denair_volpoll`        | any  | 4        | 5 Hz NVS-cache poll for iOS slider    |
...
- **Link order trick**: DenAir init calls (`denair_leds_init`, `denair_ui_init`, `denair_artwork_init`) live in `main/main.c`, not in `boards/ha_voice_pe/board.c`, because ESP-IDF's PRIV_REQUIRES ordering puts the denair_* libs before `libboards.a` on the linker's single pass. Putting references in `libmain.a` (linked last) sidesteps the undefined-reference.
- **JPEG decoding in ROM**: ESP32-S3 has tjpgd built into ROM — zero flash cost, ~15 ms per 512×512 artwork at 1/8 scale. See `components/denair_artwork/README.md`.
